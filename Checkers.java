...
/**
 * imported from http://math.hws.edu/eck/cs124/javanotes5/source/Checkers.java
 * 
...
//    */
   //moved to own file for simplicity
...
//   }  // end class CheckersMove.
   ...
   private class Board extends JPanel implements ActionListener, MouseListener {
      ...
      void doNewGame() {
		...
		public void updateGui(CheckersState state) {
			board.setBoardIntArr(state.getBoardPieces()); // Set up the pieces.
			currentPlayer = state.getPlayerTurn().getColorRedorBlack(); // RED moves first.
			legalMoves = board.getLegalMoves(state.getPlayerTurn().getColorRedorBlack()); // Get RED's legal moves.
			selectedRow = -1; // RED has not yet selected a piece to move.
			message.setText("Red:  Make your move.");
			gameInProgress = true;
			newGameButton.setEnabled(false);
      ...
      void doMakeMove(CheckersMove move) {
         ...
         if (currentPlayer == CheckersData.RED) {
            ...
            //System.out.println(legalMoves[0].toString());
            if (legalMoves == null)
            ...
            //Black, the computer, takes its turn
            computerTakeTurn();
      ...
      void computerTakeTurn() {
	      ...
	      AlphaBetaSearch<CheckersState, CheckersAction, CheckersPlayer> abSearch = AlphaBetaSearch.createFor(checkersGame);
	      CheckersState checkersState = new CheckersState(board.getBoardIntArr(), currentPlayer);
	      CheckersAction checkersAction = abSearch.makeDecision(checkersState);
	      CheckersState newState = checkersGame.getResult(checkersState, checkersAction);
	      updateGui(newState);
      }
   ...
   //These commented out classes were moved into their own files
